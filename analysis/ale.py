...
def _second_order_ale_quant(predictor, train_set, features, quantiles):
    ...
    ALE = np.zeros((len(quantiles[0])-1, len(quantiles[1])-1))  # Final ALE function
    ...
    for i in range(1, len(quantiles[0])):
        for j in range(1, len(quantiles[1])):
            ...
            if len(subset) != 0:
                ...
                ALE[i-1, j-1] += (predictor(z_up[1]) - predictor(z_up[0]) - (predictor(z_low[1]) - predictor(z_low[0]))).sum() / subset.shape[0]
