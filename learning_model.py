...
def run_model(all_data, job, job_dir = None):
    ...
    if job['train_subset'] is not None:
        ...
        print('data.head(1) first time:',data.head(1))
    else:
        data = all_data
        print('data.head(1) second time:',data.head(1))
    ...
    returns = data[job['train_target']]
    train_target = returns if is_regr else (returns > 0.0)*1.0
    return_target = data[job['return_target']]
    
    feature_data = data[job['features']]
    ...
    list_val = list(val.split(data))
    print('Print one split:', list_val[0])
    ...
    #print('printing_feature_data:', feature_data)

    #print('Print one split:',val.split(data)[0])
    #split_generator = val.split(data)
    #print('Print one split:', next(split_generator))
    ...
    for i, (train_index, test_index, val_index) in enumerate(val.split(data)):
        ...
        abs_ret = data.loc[train_index, job['train_target']].abs()
        ...
        if not is_regr:
            if 'Pipeline' in model.__class__.__name__ and model.steps[1][0] == 'ANN':
                ...
                wd = feature_data.loc[train_index].apply(sst.mstats.winsorize, limits=(0.01, 0.01))
            ...
            if 'Regression' in model.__class__.__name__ or 'Lasso' in str(model):
                ...
                #                                       index = feature_data.loc[train_index].index,
                #                                       columns = feature_data.loc[train_index].columns)
                wd= winsorized_df = feature_data.loc[train_index].apply(sst.mstats.winsorize, limits=(0.01, 0.01))
        ...
        print('Print train index',train_index)
        
        
        union_index = train_index.union(val_index).union(test_index)

        print('Print Union index', union_index)

        val_test_index = val_index.union(test_index)

        print('Print val test index', val_test_index)
        ...
        result = pd.concat([data.loc[test_index, ['filename', 'file_id', 
                            ...
                            result],
                            axis = 1)
        print('Print result:', result.head())
