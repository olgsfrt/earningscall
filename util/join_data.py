...
# -*- coding: utf-8 -*-
"""
Created on Sun Jun 16 17:26:34 2019
...
def read_call_data(call_file):
    try:
        ...
        print(f"Error: Unable to read the call file. {str(e)}")
        return pd.DataFrame()
...
def read_call_data(call_file):
    try:
        data = pd.DataFrame(call_file)
    except Exception as e:
        print(f"Error: Unable to process the call data. {str(e)}")
        return pd.DataFrame()
    ...
    # Fix to add 'Z' to timestamp
    data['iso_date'] = data.iso_date.apply(lambda x: x if (pd.isna(x) or x.endswith('Z')) else x + 'Z')
...
def get_price_data(export_folder, ticker_list):
        prices = pd.read_hdf(export_folder + 'adjusted_prices.hdf')
...
def read_and_join_data(call_file, export_folder, indices, tmp_file,
    ...
    def file_id(p):
        ...
        return "%s%d" % (mtype, int(m.groups()[1]))
    
    if 'filename' in data.columns:
        data['file_id'] = data.filename.apply(file_id)
    
    
    
    ### prepare extracted quarter and year (-> do while parsing?) ###
    
    data['year'] = data.year.apply(float)
    q_string = data.quarter.str.replace('[0-9]{2}', '').str.replace('[a-zA-z]', '').str.strip()
    q_string.loc[q_string.str.match('[1-4]') == False] = np.nan
    
    data['quarter'] = q_string.apply(lambda x: float(x) if not pd.isna(x) else np.nan)
    data.loc[data['quarter'] > 4, 'quarter'] = np.nan
    
    
    def fiscal_period(r):
        if not (pd.isna(r['year']) or pd.isna(r['quarter'])):
            return 'FY%4dQ%d' % (r['year'], r['quarter'])
        else:
            return None
    ...
    print("Found %d calls with missing release date information" % data.local_date.isna().sum())
    
    
    ### join index data to get link between SA ticker symbol and Eikon RIC/Instrument Code ###
    
    index = pd.read_hdf(export_folder + 'ticker_index.hdf')
    ...
    print("Total of %d calls could not be matched to Eikon instruments" \
          % data.primary_ric.isna().sum())
    
    print("Total of %d SA tickers could not be matched to Eikon instruments" \
          % len(data[data.primary_ric.isna()].ticker_symbol.unique()))
    ...
    mdata = mdata.groupby('ticker_symbol').aggregate({
            ...
            'file_id': ['first','last']})\
                .sort_values(('company_name','count'), ascending = False)
